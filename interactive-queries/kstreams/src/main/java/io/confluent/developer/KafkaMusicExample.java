 ...
 *      io.confluent.developer.KafkaMusicExample 7070
 * }
 * </pre>
 *
 ...
 * {@code
 * $ java -cp target/kafka-streams-examples-7.6.1-standalone.jar \
 *      io.confluent.developer.KafkaMusicExample 7071
 * }
 * </pre>
 *
...
public class KafkaMusicExample {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    Properties properties =  streamsConfig(bootstrapServers, restEndpointPort, "/tmp/kafka-streams", restEndpointHostname);
    Topology topology = buildTopology(properties);
    final KafkaStreams streams = kafkaStreams(topology, properties);
  ...
  private static KafkaStreams kafkaStreams(Topology topology, Properties properties) {
  ...
  static Topology buildTopology(Properties props) {
    ...
    final Serde<PlayEvent> playEventSerde = StreamsSerde.serdeFor(PlayEvent.class);
    final Serde<Song> songSerde = StreamsSerde.serdeFor(Song.class);
    final Serde<SongPlayCount> songPlayCountSerde = StreamsSerde.serdeFor(SongPlayCount.class);
    ...
    final KTable<Long, Song>
        ...
        builder.table(SONG_FEED, Materialized.<Long, Song, KeyValueStore<Bytes, byte[]>>as(ALL_SONGS)
            ...
            .withValueSerde(songSerde));

    // Accept play events that have a duration >= the minimum
    final KStream<Long, PlayEvent> playsBySongId =
        playEvents.filter((region, event) -> event.duration() >= MIN_CHARTABLE_DURATION)
            // repartition based on song id
            .map((key, value) -> KeyValue.pair(value.songId(), value));
    ...
    songPlayCounts.groupBy((song, plays) ->
            KeyValue.pair(song.genre().toLowerCase(),
                new SongPlayCount(song.id(), plays)),
      ...
      return (s, topFiveSongs) -> {
          ...
          try {
              for (final SongPlayCount songPlayCount : topFiveSongs) {
                  dataOutputStream.writeLong(songPlayCount.songId());
                  dataOutputStream.writeLong(songPlayCount.plays());
              }
  ...
  static class TopFiveSongs implements Iterable<SongPlayCount> {
    ...
    private final TreeSet<SongPlayCount> topFive = new TreeSet<>((o1, o2) -> {
      final Long o1Plays = o1.plays();
      final Long o2Plays = o2.plays();

      final int result = o2Plays.compareTo(o1Plays);
      if (result != 0) {
        return result;
      }
      final Long o1SongId = o1.songId();
      final Long o2SongId = o2.songId();
    ...
    public void add(final SongPlayCount songPlayCount) {
      if(currentSongs.containsKey(songPlayCount.songId())) {
        topFive.remove(currentSongs.remove(songPlayCount.songId()));
      }
      topFive.add(songPlayCount);
      currentSongs.put(songPlayCount.songId(), songPlayCount);
      if (topFive.size() > 5) {
        final SongPlayCount last = topFive.last();
        currentSongs.remove(last.songId());
        topFive.remove(last);
      }
    }
    ...
    void remove(final SongPlayCount value) {
      ...
      currentSongs.remove(value.songId());
    }
