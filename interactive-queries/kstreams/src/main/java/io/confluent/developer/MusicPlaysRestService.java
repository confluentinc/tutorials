...
public class MusicPlaysRestService {
  ...
  private boolean thisHost(final HostStoreInfo host) {
    return host.host().equals(hostInfo.host()) &&
  ...
  private List<SongPlayCountBean> topFiveSongs(final String key,
    ...
    for (final SongPlayCount songPlayCount : value) {
      ...
      if (!thisHost(host)) {
        ...
        final Song song = songStore.get(songPlayCount.songId());
        results.add(new SongPlayCountBean(song.artist(), song.album(), song.name(),
                                          songPlayCount.plays()));
      }
    }
    return results;
  ...
  public SongBean song(@PathParam("id") final Long songId) {
    ...
    return new SongBean(song.artist(), song.album(), song.name());
  }
