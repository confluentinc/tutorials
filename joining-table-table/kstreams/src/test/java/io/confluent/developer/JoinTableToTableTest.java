...
class JoinTableToTableTest {

    @Test
    @DisplayName("Should join movie table with ratings table")
    void testJoin() {
        JoinTableToTable jst = new JoinTableToTable();
        Properties allProps = new Properties();

        String tableTopic = JoinTableToTable.MOVIE_INPUT_TOPIC;
        String streamTopic = JoinTableToTable.RATING_INPUT_TOPIC;
        String outputTopic = JoinTableToTable.RATED_MOVIES_OUTPUT;
        ...
        try (TopologyTestDriver driver = new TopologyTestDriver(topology, allProps)) {
            ...
            final TestInputTopic<Long, Movie> movieTestInputTopic = driver.createInputTopic(tableTopic, keySerializer, movieSerializer);
            final TestInputTopic<Long, Rating> ratingTestInputTopic = driver.createInputTopic(streamTopic, keySerializer, ratingSerializer);
            final TestOutputTopic<Long, RatedMovie> ratedMovieTestOutputTopic = driver.createOutputTopic(outputTopic, keyDeserializer, valueDeserializer);
